...
This enhancement is to add a dynamic scoring framework addon to Open Cluster Management (OCM). The dynamic scoring framework addon enables users to define custom scoring logic for scheduling decisions in a Kubernetes cluster.
...
In these evaluation steps, time series data from each cluster are used as input to the Prediction/Evaluation/Scoring engine, and a resulted score is produced by the logic, which is then used to trigger subsequent actions. 


The DynamicScoring Framework addresses two competing requirements for efficient multi-cluster evaluation:
...
The DynamicScoring Framework resolves this by implementing a hybrid architecture:
...
The evaluation logic is expected to be implemented outside the DynamicScoring Framework.
(But interfaces for Scoring API should be defined in the framework.)

## Proposal

### User Stories

#### Story 1: AI-RAN Use Case
...
When implementing the scoring API, the operator don't need to worry about how to collect fresh metrics from each cluster or how to distribute the scoring logic, as these are handled by the Dynamic Scoring Framework.

#### Story 2: Multi-Cluster Multi-Device Inference Platform
...
User -> AFC: Install DynamicScoring Framework Addon
...
User -> DSFC: Create CR (=Set DynamicScoring Configuration)
...
If the user of the DynamicScoring Framework is also the implementer of the scoring API, this distinction is not necessary.  
...
The Scoring API calculates scores based on the data above. The schema of the response can be defined by the Scoring API implementer; however, the Dynamic Scoring Addon Agent generates metric endpoints from the received score information by mapping the labels of the metric.
...
3. Return the score calculation results while taking into account the mapping rules of the Dynamic Scoring Addon Agent:
